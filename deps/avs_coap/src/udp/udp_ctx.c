...
#include "ctx.h"
#include "ctx_vtable.h"
#include "options/option.h"
#include "options/options.h"

#include "udp/udp_header.h"
#include "udp/udp_msg.h"
#include "udp/udp_msg_cache.h"
 ...
 */
typedef struct {
    avs_coap_udp_sent_notify_t entries[COAP_UDP_NOTIFY_CACHE_SIZE];
    size_t size;
} avs_coap_udp_notify_cache_t;

AVS_STATIC_ASSERT(COAP_UDP_NOTIFY_CACHE_SIZE > 0,
...
static avs_time_monotonic_t get_first_retransmit_time(avs_coap_udp_ctx_t *ctx) {
    const avs_coap_retry_state_t initial_state =
            _avs_coap_udp_initial_retry_state(&ctx->tx_params,
                                              &ctx->base.rand_seed);
...
static void resume_next_unconfirmed(avs_coap_udp_ctx_t *ctx) {
    ...
    if (!avs_time_monotonic_valid(next_retransmit)) {
        ...
        do {
            ...
            // avs_coap_async_send_request, adding a new held entry to the
            // context.
            avs_coap_udp_unconfirmed_msg_t *unconfirmed =
...
static void coap_udp_ignore_current_request(avs_coap_ctx_t *ctx,
                                            const avs_coap_token_t *token) {
    (void) ctx;
    (void) token;
    // No-op - in UDP context message is received in one call to receive_message
    // method.
}
...
retransmit_next_message_without_reschedule(avs_coap_udp_ctx_t *ctx) {
    ...
    // result intentionally ignored, unable to pass it up to the caller
    (void) coap_udp_send_serialized_msg(ctx, &unconfirmed->msg,
                                        unconfirmed->packet,
                                        unconfirmed->packet_size);
...
static avs_error_t create_unconfirmed(
    ...
    *unconfirmed_msg = (avs_coap_udp_unconfirmed_msg_t) {
        ...
        .retry_state = _avs_coap_udp_initial_retry_state(&ctx->tx_params,
                                                         &ctx->base.rand_seed),
...
static avs_error_t handle_response(avs_coap_udp_ctx_t *ctx,
    ...
    case AVS_COAP_UDP_TYPE_CONFIRMABLE:
        ...
        // return value deliberately ignored (what can we do with it?)
        send_separate_ack(ctx, _avs_coap_udp_header_get_id(&msg->header));
        break;
...
static void send_empty_response(avs_coap_udp_ctx_t *ctx,
    ...
    (void) coap_udp_send_serialized_msg(ctx, &msg, buf, msg_size);
}

static void handle_truncated_request(avs_coap_udp_ctx_t *ctx,
                                     const avs_coap_udp_msg_t *truncated_msg) {
    log_udp_msg_summary("recv [truncated request]", truncated_msg);
    assert(avs_coap_code_is_request(truncated_msg->header.code));
    send_empty_response(ctx, truncated_msg,
...
static avs_error_t handle_truncated_msg(avs_coap_udp_ctx_t *ctx,
    ...
    if (avs_coap_code_is_request(truncated_msg->header.code)) {
        handle_truncated_request(ctx, truncated_msg);
    ...
    if (err.category == AVS_COAP_ERR_CATEGORY) {
        ...
        case AVS_COAP_ERR_MALFORMED_OPTIONS: {
            if (avs_coap_code_is_request(msg.header.code)) {
                ...
                send_empty_response(ctx, &msg, AVS_COAP_CODE_BAD_OPTION);
            } else if (avs_coap_code_is_response(msg.header.code)) {
...
avs_coap_udp_ctx_create(avs_sched_t *sched,
    ...
    ctx->last_msg_id = (uint16_t) avs_rand_r(&ctx->base.rand_seed);
