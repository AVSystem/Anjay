...
allocate_mocksock(anjay_t *anjay,
    ...
    AVS_UNIT_ASSERT_TRUE(env->num_mocksocks < ANJAY_ARRAY_SIZE(env->mocksock));
...
static void setup(dl_test_env_t *env) {
    *env = (dl_test_env_t){0};
    ...
    for (size_t i = 0; i < ANJAY_ARRAY_SIZE(env->mocksock); ++i) {
    ...
    AVS_UNIT_ASSERT_SUCCESS(_anjay_coap_socket_create(&env->anjay.coap_socket,
                                                      NULL, 0));
...
static void teardown(dl_test_env_t *env) {
    ...
    _anjay_coap_socket_cleanup(&env->anjay.coap_socket);
...
AVS_UNIT_TEST(downloader, coap_download_single_block) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *res = COAP_MSG(ACK, CONTENT, ID(0),
    ...
    avs_unit_mocksock_expect_output(env.mocksock,
                                    &req->header, req->length);
    avs_unit_mocksock_input(env.mocksock, &res->header, res->length);
...
AVS_UNIT_TEST(downloader, coap_download_multiple_blocks) {
    ...
    for (size_t i = 0; i < num_blocks; ++i) {
        const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(i),
                                               BLOCK2(i, i == 0 ? 1024 : BLOCK_SIZE));
        const anjay_coap_msg_t *res = COAP_MSG(ACK, CONTENT, ID(i),
...
AVS_UNIT_TEST(downloader, download_abort_on_reset_response) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *res = COAP_MSG(RST, EMPTY, ID(0), NO_PAYLOAD);

    avs_unit_mocksock_expect_connect(env.mocksock, "127.0.0.1", "5683");
    avs_unit_mocksock_expect_output(env.mocksock, &req->header, req->length);
    avs_unit_mocksock_input(env.mocksock, &res->header, res->length);
...
AVS_UNIT_TEST(downloader, coap_download_separate_response) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *res = COAP_MSG(CON, CONTENT, ID(1),
                                           BLOCK2(0, 128, DESPAIR));
    const anjay_coap_msg_t *res_res = COAP_MSG(ACK, EMPTY, ID(1), NO_PAYLOAD);

    avs_unit_mocksock_expect_connect(env.mocksock, "127.0.0.1", "5683");
    avs_unit_mocksock_expect_output(env.mocksock, &req->header, req->length);
    avs_unit_mocksock_input(env.mocksock, &res->header, res->length);
    avs_unit_mocksock_expect_output(env.mocksock,
                                    &res_res->header, res_res->length);
...
AVS_UNIT_TEST(downloader, coap_download_unexpected_packet) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *unk1 = COAP_MSG(RST, CONTENT, ID(1), NO_PAYLOAD);
    const anjay_coap_msg_t *unk2 = COAP_MSG(NON, CONTENT, ID(2), NO_PAYLOAD);
    const anjay_coap_msg_t *res = COAP_MSG(ACK, CONTENT, ID(0),
                                           BLOCK2(0, 128, DESPAIR));

    avs_unit_mocksock_expect_connect(env.mocksock, "127.0.0.1", "5683");
    avs_unit_mocksock_expect_output(env.mocksock, &req->header, req->length);
    avs_unit_mocksock_input(env.mocksock, &unk1->header, unk1->length);
    avs_unit_mocksock_input(env.mocksock, &unk2->header, unk2->length);
    avs_unit_mocksock_input(env.mocksock, &res->header, res->length);
...
AVS_UNIT_TEST(downloader, coap_download_expired) {
    ...
    // expect packets
    const anjay_coap_msg_t *req1 = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *res1 = COAP_MSG(ACK, CONTENT, ID(0), ETAG("tag"),
                                            BLOCK2(0, 64, DESPAIR));

    const anjay_coap_msg_t *req2 = COAP_MSG(CON, GET, ID(1), BLOCK2(1, 64));
    const anjay_coap_msg_t *res2 = COAP_MSG(ACK, CONTENT, ID(1), ETAG("nje"),
                                            BLOCK2(1, 64, DESPAIR));

    avs_unit_mocksock_expect_connect(env.mocksock, "127.0.0.1", "5683");
    avs_unit_mocksock_expect_output(env.mocksock, &req1->header, req1->length);
    avs_unit_mocksock_input(env.mocksock, &res1->header, res1->length);
    avs_unit_mocksock_expect_output(env.mocksock, &req2->header, req2->length);
    avs_unit_mocksock_input(env.mocksock, &res2->header, res2->length);
...
AVS_UNIT_TEST(downloader, retry) {
    ...
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *res = COAP_MSG(ACK, CONTENT, ID(0), ETAG("tag"),
                                           BLOCK2(0, 128, DESPAIR));
...
AVS_UNIT_TEST(downloader, missing_separate_response) {
    ...
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 1024));
    const anjay_coap_msg_t *req_ack = COAP_MSG(ACK, EMPTY, ID(0), NO_PAYLOAD);
    ...
    // separate ACK
    avs_unit_mocksock_input(env.mocksock, &req_ack->header, req_ack->length);
    ...
    ASSERT_ALMOST_EQ(timespec_to_s(&time_to_next),
                     (double) _anjay_coap_exchange_lifetime_ms(&tx_params)
                             / 1000.0);
...
AVS_UNIT_TEST(downloader, uri_path_query) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0),
                                           PATH("uri", "path"),
...
AVS_UNIT_TEST(downloader, in_buffer_size_enforces_smaller_initial_block_size) {
    ...
    // expect packets
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 128));
    const anjay_coap_msg_t *res = COAP_MSG(ACK, CONTENT, ID(0),
                                           BLOCK2(0, 128, DESPAIR));
...
AVS_UNIT_TEST(downloader, renegotiation_after_first_packet) {
    ...
    const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(0), BLOCK2(0, 64));
...
AVS_UNIT_TEST(downloader, resumption_at_some_offset) {
    for (size_t offset = 0; offset < sizeof(DESPAIR); ++offset) {
        ...
        while (sizeof(DESPAIR) - current_offset > 0) {
            ...
            const anjay_coap_msg_t *req = COAP_MSG(CON, GET, ID(msg_id),
                                                   BLOCK2(seq_num, BLOCK_SIZE));
            const anjay_coap_msg_t *res =
                    COAP_MSG(ACK, CONTENT, ID(msg_id),
                             BLOCK2(seq_num, BLOCK_SIZE, DESPAIR));
            avs_unit_mocksock_expect_output(env.mocksock, &req->header,
                                            req->length);
            avs_unit_mocksock_input(env.mocksock, &res->header, res->length);
            ...
            const size_t bytes_till_block_end =
                    ANJAY_MIN((seq_num + 1) * BLOCK_SIZE - current_offset,
