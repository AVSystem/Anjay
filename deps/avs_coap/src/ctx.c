...
#include "ctx.h"
#include "options/iterator.h"
#include "options/options.h"

#include "async/async_client.h"
#include "async/exchange.h"
...
avs_coap_token_t _avs_coap_ctx_generate_token(avs_coap_ctx_t *ctx) {
    /**
     * TODO: it is possible to expand the space of available tokens by using
     * different token lengths, i.e.
     * - use zero-length token first
     * - after using it, use all 2**8 1-byte tokens
     * - after using them, use 2**16 2-byte tokens
     * - etc.
     */
    ...
    uint64_t value = avs_rand64_r(&_avs_coap_get_base(ctx)->rand_seed);
    avs_unaligned_put(token.bytes, value);
