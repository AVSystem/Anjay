...
#define CHUNK_SIZE 256
        for (size_t offset = 0; offset < size; offset += CHUNK_SIZE) {
            ...
            avs_hexlify(buf, sizeof(buf), (const char *) buffer + offset,
                        size - offset);
...
static avs_error_t mock_recv(avs_net_socket_t *socket,
    ...
    *out_bytes_received = AVS_MIN(*out_bytes_received, buffer_length);
    ...
    if (read_flag() && buffer_length >= 4) {
        LOG(DEBUG, "mock_recv: echo ID");
        // echo message ID & token
        memcpy(&(((char *) buffer)[2]), &g_last_send[2],
               AVS_MIN(buffer_length - 2, 2));
        size_t toklen = g_last_send[0] & 0xF;
        if (toklen <= 8) {
            LOG(DEBUG, "mock_recv: echo token (%u B)", (unsigned) toklen);
            memcpy(&(((char *) buffer)[4]), &g_last_send[4],
                   AVS_MIN(buffer_length - 4, toklen));
        }
    }
...
static int handle_request(avs_coap_request_ctx_t *ctx,
    ...
    if (ctx) {
        ...
        LOG(DEBUG, "handle_request: setup_async_response: %s",
...
static void do_stuff_unconditionally(avs_coap_ctx_t *ctx) {
    ...
    case OP_SEND_ASYNC_REQUEST: {
        ...
        LOG(DEBUG, "avs_coap_async_send_request");
    ...
    case OP_HANDLE_INCOMING_PACKET: {
        LOG(DEBUG, "avs_coap_handle_incoming_packet");
