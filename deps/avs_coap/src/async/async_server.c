...
send_response_to_request_chunk(avs_coap_ctx_t *ctx,
    ...
    if (avs_is_err(err) || coap_base->request_ctx.response_setup) {
        // Prevent user handler to be called again for this message if message
        // is set up by user or by us or setting up a response failed, which
        // results in sending an empty message.
        ctx->vtable->ignore_current_request(
                ctx, &coap_base->request_ctx.request.token);
    }
...
void _avs_coap_server_exchange_cleanup(avs_coap_ctx_t *ctx,
    ...
    ctx->vtable->ignore_current_request(ctx, &exchange->token);
