...
AVS_UNIT_TEST(tlv_out, bytes_3blen_16bid) {
    ...
    AVS_UNIT_ASSERT_SUCCESS(anjay_ret_string(out, DATA));
    AVS_UNIT_ASSERT_SUCCESS(_anjay_output_ctx_destroy(&out));
    VERIFY_BYTES("\xE7\xA4\x10" "1234567");
}
...
AVS_UNIT_TEST(tlv_out, bytes_8blen_8bid) {
    ...
    AVS_UNIT_ASSERT_SUCCESS(anjay_ret_string(out, DATA));
    AVS_UNIT_ASSERT_SUCCESS(_anjay_output_ctx_destroy(&out));
    VERIFY_BYTES("\xC8\xFF\x08" "12345678");
}
...
AVS_UNIT_TEST(tlv_out, bytes_8blen_16bid) {
    ...
    AVS_UNIT_ASSERT_SUCCESS(anjay_ret_string(out, DATA));
    AVS_UNIT_ASSERT_SUCCESS(_anjay_output_ctx_destroy(&out));
    VERIFY_BYTES("\xE8\xFF\xFE\x08" "12345678");
}
...
TEST_INT3264(                  42, "\xC1\x01"                              "\x2A")
TEST_INT3264(                4242, "\xC2\x01"                          "\x10\x92")
TEST_INT3264(              424242, "\xC4\x01"                  "\x00\x06\x79\x32")
TEST_INT3264(            42424242, "\xC4\x01"                  "\x02\x87\x57\xB2")
TEST_INT3264((int32_t) 4242424242, "\xC4\x01"                  "\xFC\xDE\x41\xB2")
