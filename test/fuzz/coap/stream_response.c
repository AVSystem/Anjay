...
static int mock_recv(avs_net_abstract_socket_t *socket,
    ...
    //      |  |  token length
    //      v  v  v     .- code .
    //      01 00 0000  000 00000
    // hex:     6    0      0   0
#define EMPTY_CONFIRMABLE "\x40\x00"

    *out_bytes_received = 4;
    memcpy(buffer, EMPTY_CONFIRMABLE, sizeof(EMPTY_CONFIRMABLE) - 1);
    uint16_t id = ((mock_socket_t*)socket)->last_msg_id;
    memcpy((char*)buffer + sizeof(EMPTY_CONFIRMABLE) - 1, &id, sizeof(id));
    return 0;
}
...
static const avs_net_socket_v_table_t MOCK_SOCKET_VTABLE = {
    ...
    .send               = (avs_net_socket_send_t)            success,
    ...
    case OP_SETUP_RESPONSE:
        {
            ...
            _anjay_coap_stream_setup_response(stream, &details);
        }
    case OP_WRITE:
        {
            uint16_t size;
