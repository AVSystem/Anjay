...
/**
 * Allow Anjay to use unsecured connections.
 *
 * If this flag is disabled, unsecured connections cannot be used for
 * management, bootstrap, or firmware update operations. Anjay will also not
 * allow connection to server using certificates security mode while no server
 * cert or trust store is configured.
 *
 * This option is intended for development use only and should not be enabled
 * in production builds.
 */
/* #undef ANJAY_WITH_UNSECURE_CONNECTIONS */
...
/**
 * Size, in bytes, of the buffer used for generating and storing a private key
 * during EST enrollment.
 *
 * The buffer must be large enough to hold the private key generated by the
 * configured cryptographic backend. This setting is not used when the key is
 * generated and stored in a cryptographic engine.
 *
 * The default value defined in CMake build scripts is 256.
 * Minimal suggested setting for low-resource builds is 128.
 *
 * Only meaningful if <c>ANJAY_WITH_EST</c> is enabled.
 */
/* #undef ANJAY_EST_SECRET_KEY_BUFFER_SIZE */

/**
 * Size, in bytes, of the buffer allocated for generating an EST certificate
 * signing request.
 *
 * CSR generation will fail if the encoded CSR does not fit in this buffer.
 *
 * The default value defined in CMake build scripts is 2048.
 * Minimal suggested setting for low-resource builds is 512.
 *
 * Only meaningful if <c>ANJAY_WITH_EST</c> is enabled.
 */
/* #undef ANJAY_EST_CSR_BUFFER_SIZE */
