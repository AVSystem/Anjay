...
test_setup_without_socket(const avs_coap_udp_tx_params_t *tx_params,
    ...
    // manually crafted constant that results in initializing first msg_id to 0
    _avs_mock_clock_start(avs_time_monotonic_from_scalar(42627, AVS_TIME_S));
 ...
 */
static inline void expect_request_handler_call(
        test_env_t *env,
        avs_coap_server_request_state_t state,
...
test_accept_new_request(avs_coap_server_ctx_t *ctx,
    ...
    avs_coap_exchange_id_t id =
            avs_coap_server_accept_async_request(ctx, test_handle_request, env);
