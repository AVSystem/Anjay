...
#ifdef EXAMPLE_WITH_SEND_OPERATION
#    include <anj/sdm_send.h>
#endif // EXAMPLE_WITH_SEND_OPERATION
...
static void handle_server_request(event_loop_ctx_t *ctx) {
    ...
    if (ctx->msg.operation == FLUF_OP_DM_READ
        ...
        int res = sdm_process(&ctx->dm_impl, &ctx->dm, &ctx->msg, false,
                              ctx->payload, EXAMPLE_PAYLOAD_BUFFER_SIZE);
...
static int open_connection(event_loop_ctx_t *ctx) {
    ...
    const char *identity =
            ctx->security_obj.security_instances[0].public_key_or_identity;
    const char *psk = ctx->security_obj.security_instances[0].secret_key;
    anj_net_op_res_t res =
...
#ifdef EXAMPLE_WITH_SEND_OPERATION
static void send_operation_process(event_loop_ctx_t *ctx) {
    fluf_uri_path_t path[] = { EXAMPLE_SEND_RECORD_PATH };
    // sdm_send doesn't support block transfers yet
    ctx->payload_size = EXAMPLE_PAYLOAD_BUFFER_SIZE;
    if (sdm_send_create_msg_from_dm(&ctx->dm,
                                    EXAMPLE_SEND_CONTENT_FORMAT,
                                    (double) (anj_time_real_now()
                                              / (uint64_t) 1000),
                                    ctx->payload,
                                    &ctx->payload_size,
                                    path,
                                    AVS_ARRAY_SIZE(path))) {
        event_loop_log(ERROR, "sdm_send_create_msg_from_dm failed");
        return;
    }
    memset(&ctx->msg, 0, sizeof(ctx->msg));
    ctx->msg.operation = FLUF_OP_INF_NON_CON_SEND;
    ctx->msg.binding = FLUF_BINDING_UDP;
    ctx->msg.payload_size = ctx->payload_size;
    ctx->msg.payload = ctx->payload;
    ctx->msg.content_format = EXAMPLE_SEND_CONTENT_FORMAT;
...
#ifdef EXAMPLE_WITH_SEND_OPERATION
        // no ongoing transaction in this iteration of the event loop
        if (ctx->state == EVENT_LOOP_STATE_IDLE) {
            // send message every EXAMPLE_SEND_INTERVAL_MS
            if ((anj_time_now()
                 > ctx->last_send_timestamp + EXAMPLE_SEND_INTERVAL_MS)
#    ifdef EXAMPLE_WITH_QUEUE_MODE
                    && (anj_time_now()
                        < ctx->last_update_timestamp
                                  + EXAMPLE_TIME_WINDOW_AFTER_UPDATE_MS)
#    endif // EXAMPLE_WITH_QUEUE_MODE
            ) {
                ctx->last_send_timestamp = anj_time_now();
                send_operation_process(ctx);
            }
        }
#endif // EXAMPLE_WITH_SEND_OPERATION
        break;
