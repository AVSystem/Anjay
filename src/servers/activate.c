...
void _anjay_server_on_failure(anjay_server_info_t *server,
    ...
    if (server->ssid == ANJAY_SSID_BOOTSTRAP) {
        ...
        bool should_retry_bootstrap =
                _anjay_should_retry_bootstrap(server->anjay);
        if (should_retry_bootstrap) {
...
bool _anjay_should_retry_bootstrap(anjay_t *anjay) {
    ...
    return bootstrap_server_exists && !possibly_active_server_exists;
#else  // WITH_BOOTSTRAP
    (void) anjay;
    return false;
