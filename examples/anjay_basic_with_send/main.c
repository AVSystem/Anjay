...
int main(int argc, char *argv[]) {
    ...
    uint64_t last_send = anj_time_now();
    while (1) {
        anjay_lite_process(&anjay_lite);

        /* Generate Send message every 10 sec. */
        if (anj_time_now() > last_send + 10 * 1000) {
            fluf_uri_path_t path[] = { FLUF_MAKE_RESOURCE_PATH(3303, 0, 5700) };
            uint8_t payload[50];
            size_t payload_size = sizeof(payload);

            if (!sdm_send_create_msg_from_dm(&anjay_lite.dm,
                                             FLUF_COAP_FORMAT_SENML_CBOR,
                                             payload,
                                             &payload_size,
                                             path,
                                             AVS_ARRAY_SIZE(path))) {
                anjay_lite_send(payload, payload_size);
                last_send = anj_time_now();
            }
        }
        usleep(50 * 1000);
    }
